...
- (void)drawRect:(CGRect)rect
{
    ...
    if (CLSliderStyle_Nomal == self.sliderStyle || CLSliderStyle_Cross == self.sliderStyle) {
        ...
        for (NSInteger i = 0; i < lineNum; i ++) {
			...
			CGSize strSize = [self.markDataArr[i] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:10.0f]}];
		// 写文本
			[self.markDataArr[i] drawInRect:CGRectMake(startP.x -strSize.width /2, endY + 8, strSize.width, strSize.height)
         	withAttributes:@{NSForegroundColorAttributeName :self.scaleLineColor,
